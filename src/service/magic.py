...
class Wormhole(QObject):
    ...
    def __init__(
            self, _reactor=None, app_id=APPID, rendezvous_relay=RENDEZVOUS_RELAY,
        ...
        try:
            self.wormhole = create(self.app_id, self.rendezvous_relay, self.reactor)
        except Exception:
    ...
    def  generate_code(self, timeout=10):
        ...
        """
        self.wormhole.allocate_code(4)

        deferred = self.wormhole.get_code()
        deferred.addTimeout(timeout, self.reactor)
